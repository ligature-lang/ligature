...
use ligature_eval::{evaluate_expression, CacheStats};
...
fn main() {
    ...
    println!("  Cache size: {}/{}", evaluator.cache_size(), evaluator.cache_size_limit());
    println!();
    ...
    println!("  Cache size: {}/{}", evaluator2.cache_size(), evaluator2.cache_size_limit());
    println!();
    ...
    println!("  After first evaluation - hits: {}, misses: {}", stats3.hits, stats3.misses);
    
    // Clear cache
    evaluator3.clear_cache();
    let stats3_after_clear = evaluator3.cache_stats();
    println!("  After clearing cache - hits: {}, misses: {}", stats3_after_clear.hits, stats3_after_clear.misses);
    println!();
    ...
    for (i, expr_str) in expressions.iter().enumerate() {
        ...
        let duration = start.elapsed();
        println!("  Expression {}: {} = {:?} (took {:?})", i + 1, expr_str, result, duration);
    }
    ...
    println!("    Cache size: {}/{}", evaluator4.cache_size(), evaluator4.cache_size_limit());
