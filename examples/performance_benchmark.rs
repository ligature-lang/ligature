...
use ligature_ast::{AstResult, Expr, ExprKind, Literal, Span, BinaryOperator};
...
fn benchmark_function_calls() -> AstResult<()> {
    println!("\n1. Function Call Performance Benchmark");
    println!("-------------------------------------");
...
fn benchmark_arithmetic_operations() -> AstResult<()> {
...
fn benchmark_expression_caching() -> AstResult<()> {
    ...
    println!("Cache hit rate: {:.2}%", stats.hit_rate());
    println!("Speedup: {:.2}x", warmup_duration.as_secs_f64() / cache_duration.as_secs_f64());
...
fn benchmark_value_representation() -> AstResult<()> {
    ...
    // Test value creation and cloning performance
    let mut evaluator = Evaluator::new();
    ...
    for i in 0..iterations {
        ...
        fields.insert("x".to_string(), ligature_eval::Value::integer(i, Span::default()));
        fields.insert("y".to_string(), ligature_eval::Value::string(format!("value_{}", i), Span::default()));
        let value = ligature_eval::Value::record(fields, Span::default());
        let _cloned = value.clone();
    }
    let record_duration = start.elapsed();
    
    println!("Integer operations per second: {:.0}", iterations as f64 / int_duration.as_secs_f64());
    println!("String operations per second: {:.0}", iterations as f64 / string_duration.as_secs_f64());
    println!("Record operations per second: {:.0}", iterations as f64 / record_duration.as_secs_f64());
    
    let (available, total) = evaluator.value_pool_stats();
    println!("Value pool utilization: {}/{} ({:.1}%)", available, total, (available as f64 / total as f64) * 100.0);
    
    Ok(())
}

fn benchmark_memory_usage() -> AstResult<()> {
    println!("\n5. Memory Usage Performance Benchmark");
    println!("------------------------------------");
    ...
    for i in 0..iterations {
        let expr = create_nested_expression(i);
    ...
    println!("Cache memory usage: {} bytes", evaluator.cache_memory_usage());
    println!("Environment pool: {}/{} ({:.1}%)", env_available, env_total, (env_available as f64 / env_total as f64) * 100.0);
    println!("Value pool: {}/{} ({:.1}%)", value_available, value_total, (value_available as f64 / value_total as f64) * 100.0);
