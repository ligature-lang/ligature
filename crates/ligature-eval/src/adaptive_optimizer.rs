...
use crate::performance::{
    PerformanceMonitor, PerformanceMetrics, ExpressionProfile, 
...
pub struct AdaptiveOptimizer {
    strategies: Arc<Mutex<HashMap<String, AdaptiveOptimizationStrategy>>>,
...
impl AdaptiveOptimizer {
    ...
    fn initialize_strategies(&mut self) {
        ...
        strategies.insert(
            ...
            AdaptiveOptimizationStrategy {
                ...
                strategy: OptimizationStrategy::IncreaseCacheSize { target_hit_rate: 0.9 },
        ...
        strategies.insert(
            ...
            AdaptiveOptimizationStrategy {
                ...
                strategy: OptimizationStrategy::EnableExpressionCaching { complexity_threshold: 50 },
                effectiveness_score: 0.5,
                application_count: 0,
                success_count: 0,
                last_applied: None,
        ...
        strategies.insert(
            ...
            AdaptiveOptimizationStrategy {
                ...
                strategy: OptimizationStrategy::OptimizeMemoryAllocation { target_memory_usage: 50 * 1024 * 1024 },
                effectiveness_score: 0.5,
                application_count: 0,
                success_count: 0,
                last_applied: None,
        ...
        strategies.insert(
            ...
            AdaptiveOptimizationStrategy {
                ...
                strategy: OptimizationStrategy::DisableOptimization { optimization_name: "unknown".to_string() },
                effectiveness_score: 0.5,
                application_count: 0,
                success_count: 0,
                last_applied: None,
    ...
    fn calculate_system_load(&self, profiles: &HashMap<String, ExpressionProfile>, metrics: &[PerformanceMetrics]) -> SystemLoad {
        ...
        let memory_usage = (active_expressions as f64 / total_expressions.max(1) as f64 * 100.0).min(100.0);
    ...
    fn make_optimization_decisions(&self, state: &PerformanceState) -> Vec<OptimizationDecision> {
        ...
        if let Ok(strategies) = self.strategies.lock() {
            for (_strategy_id, strategy) in strategies.iter() {
                ...
                if let Some(last_applied) = strategy.last_applied {
                    let cooldown_duration = Duration::from_secs(self.config.strategy_cooldown_seconds);
                    if last_applied.elapsed().unwrap_or(Duration::ZERO) < cooldown_duration {
                        continue;
                    }
                }

                let (confidence, expected_improvement, reasoning) = self.evaluate_strategy(strategy, state);
        ...
        decisions.sort_by(|a, b| b.expected_improvement.partial_cmp(&a.expected_improvement).unwrap());
    ...
    fn evaluate_strategy(
        ...
        match &strategy.strategy {
            ...
            OptimizationStrategy::EnableExpressionCaching { complexity_threshold } => {
                let complex_expressions = state.expression_profiles.values()
                ...
                let reasoning = format!(
                    "{} of {} expressions are complex (>{})",
                    complex_expressions, total_expressions, complexity_threshold
                );
                
                (confidence, expected_improvement, reasoning)
            }
            
            OptimizationStrategy::OptimizeMemoryAllocation { target_memory_usage } => {
    ...
    fn apply_optimization(&mut self, decision: &OptimizationDecision) {
        if let Ok(mut strategies) = self.strategies.lock() {
            if let Some(strategy) = strategies.get_mut(&decision.strategy_name) {
                ...
                println!("Applied optimization: {} (confidence: {:.1}%, expected improvement: {:.1}%)", 
    ...
    pub fn learn_from_results(&mut self, before_metrics: &[PerformanceMetrics], after_metrics: &[PerformanceMetrics]) {
        ...
        if let Ok(mut strategies) = self.strategies.lock() {
            if let Ok(decisions) = self.decisions.lock() {
                for decision in decisions.iter().rev().take(5) { // Consider last 5 decisions
                    if let Some(strategy) = strategies.get_mut(&decision.strategy_name) {
                        // Update effectiveness score using exponential moving average
                        let alpha = self.learning_rate;
                        strategy.effectiveness_score = (1.0 - alpha) * strategy.effectiveness_score + alpha * improvement;
                        ...
                        strategy.average_improvement = (1.0 - alpha) * strategy.average_improvement + alpha * improvement;
                        ...
                        if strategy.effectiveness_score < 0.1 && strategy.application_count > 10 {
                            ...
                            println!("Disabled ineffective strategy: {} (effectiveness: {:.1}%)", 
                                strategy.name, strategy.effectiveness_score * 100.0);
                        }
                    }
                }
            }
    ...
    fn calculate_performance_improvement(&self, before: &[PerformanceMetrics], after: &[PerformanceMetrics]) -> f64 {
        ...
        let before_avg_time: Duration = before.iter().map(|m| m.execution_time).sum::<Duration>() / before.len() as u32;
        let after_avg_time: Duration = after.iter().map(|m| m.execution_time).sum::<Duration>() / after.len() as u32;

        if before_avg_time.as_nanos() > 0 {
            let improvement = (before_avg_time.as_nanos() as f64 - after_avg_time.as_nanos() as f64) 
                / before_avg_time.as_nanos() as f64;
            improvement.max(-1.0).min(1.0) // Clamp between -100% and +100%
        } else {
            0.0
        }
    }
    ...
    pub fn generate_optimization_report(&self) -> OptimizationReport {
        ...
        let avg_effectiveness: f64 = strategies.values()
...
impl OptimizationReport {
    ...
    pub fn print_summary(&self) {
        ...
        println!("Average effectiveness: {:.1}%", self.average_effectiveness * 100.0);
