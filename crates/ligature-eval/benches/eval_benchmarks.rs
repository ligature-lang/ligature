...
fn bench_evaluator(c: &mut Criterion) {
    ...
    for case in eval_benchmark_cases() {
        ...
        group.bench_with_input(
            ...
            |b, input| {
                b.iter(|| {
                    let _result: AstResult<Value> = parse_expression(input)
...
fn bench_large_eval_expressions(c: &mut Criterion) {
    ...
    let large_arithmetic = (1..50).map(|i| i.to_string()).collect::<Vec<_>>().join(" + ");
    let large_list = format!("[{}]", (1..50).map(|i| i.to_string()).collect::<Vec<_>>().join(", "));
    let large_record = format!("{{{}}}", (1..25).map(|i| format!("x{}: {}", i, i)).collect::<Vec<_>>().join(", "));
...
fn bench_end_to_end(c: &mut Criterion) {
    ...
    let end_to_end_cases = vec![
        ("simple_config", "let config = { port: 8080, host: \"localhost\" } in config.port"),
        ("conditional_config", "let env = \"production\" in if env == \"production\" then { debug: false, port: 443 } else { debug: true, port: 3000 }"),
        ("nested_config", "let base = { timeout: 30 } in let extended = { base, retries: 3 } in extended.timeout * extended.retries"),
    ];
