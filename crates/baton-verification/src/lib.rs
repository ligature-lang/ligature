...
use baton_specification::prelude::*;
...
pub struct VerificationEngine {
    ...
    client: Arc<LeanClient>,
    /// Specification manager
    specification: Arc<LeanSpecification>,
    /// Cache for verification results
    cache: Arc<Mutex<HashMap<String, CachedResult>>>,
    /// Configuration options
    config: VerificationConfig,
    /// Performance metrics
    metrics: Arc<Mutex<VerificationMetrics>>,
    /// Active verification tasks
    active_tasks: Arc<Mutex<HashMap<String, VerificationTask>>>,
}
...
impl VerificationEngine {
    ...
    pub fn with_config(config: VerificationConfig) -> BatonResult<Self> {
        let client = Arc::new(LeanClient::with_config(config.client_config.clone())?);
        ...
        Ok(Self {
            ...
            config,
            metrics: Arc::new(Mutex::new(VerificationMetrics::default())),
    ...
    pub async fn verify_evaluation(
        ...
        self.execute_verification(request, &cache_key, "evaluation").await
    }
    ...
    pub async fn verify_type_check(
        ...
        self.execute_verification(request, &cache_key, "type_check").await
    }
    ...
    pub async fn run_differential_test(
        ...
        self.execute_verification(request, &cache_key, "differential_test").await
    }
    ...
    pub async fn verify_type_safety(
        ...
        self.execute_verification(request, &cache_key, "type_safety").await
    }
    ...
    pub async fn verify_termination(
        ...
        self.execute_verification(request, &cache_key, "termination").await
    }
    ...
    pub async fn verify_determinism(
        ...
        self.execute_verification(request, &cache_key, "determinism").await
    }
    ...
    pub async fn verify_theorem(
        ...
        self.execute_verification(request, &cache_key, "theorem").await
    }
    ...
    pub async fn verify_invariant(
        ...
        self.execute_verification(request, &cache_key, "invariant").await
    }
    ...
    pub async fn check_specification(&self, specification_file: &str) -> BatonResult<bool> {
        let request = VerificationRequest::new(LeanRequest::CheckSpecification {
            specification_file: specification_file.to_string(),
            check_type: SpecificationCheckType::All,
        });
    ...
    pub async fn build_specification(&self) -> BatonResult<BuildStatus> {
        let mut spec = LeanSpecification::new_default()?;
    ...
    pub async fn get_cache_stats(&self) -> (usize, usize) {
        ...
        (cache_size, total_requests as usize)
    }
    ...
    async fn execute_verification(
        ...
        if self.config.enable_cache {
            if let Some(cached_response) = self.get_cached_result(cache_key).await {
                self.update_metrics(operation_type, start_time.elapsed(), true, None).await;
        ...
        self.update_metrics(operation_type, duration, false, error).await;
        ...
        for attempt in 0..=self.config.retry_config.max_attempts {
            match self.client.verify(request.clone()).await {
                ...
                Err(e) => {
                    ...
                    if attempt < self.config.retry_config.max_attempts && self.is_retryable_error(&e) {
    ...
    fn is_retryable_error(&self, error: &BatonError) -> bool {
        ...
        self.config.retry_config.retryable_errors.iter().any(|retryable| {
    ...
    async fn cache_result(&self, cache_key: &str, response: VerificationResponse) {
        ...
        cache.insert(cache_key.to_string(), CachedResult {
            response,
    ...
    async fn update_metrics(
        ...
        if error.is_none() {
            ...
            if metrics.total_verifications > 0 {
                let total_time = metrics.average_verification_time.as_millis() as u64 * (metrics.total_verifications - 1) + duration.as_millis() as u64;
                metrics.average_verification_time = Duration::from_millis(total_time / metrics.total_verifications);
            }

        // Update operation-specific times
        let entry = metrics.operation_times.entry(operation_type.to_string()).or_insert(Duration::ZERO);
...
pub mod prelude {
    pub use super::{
        VerificationEngine, VerificationConfig, RetryConfig, VerificationMetrics,
        EngineHealthStatus,
    };
} 