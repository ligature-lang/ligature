...
fn test_parametric_instances() {
    let source = r#"
        ...
        instance Eq (Eq 'a => Maybe 'a) where
            eq = \x y -> true;
    "#;
...
fn test_complex_type_class_example() {
    ...
    // The parser now correctly handles unary operators like (-7) with proper precedence
    let result = match program {
        Ok(program) => type_check_program(&program),
        Err(e) => {
            println!("⚠️  Complex type class example failed: {e}");
