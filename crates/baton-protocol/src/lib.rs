...
impl std::fmt::Display for LeanRequest {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            LeanRequest::VerifyEvaluation { expression, .. } => {
                write!(f, "VerifyEvaluation({})", expression)
            }
            LeanRequest::VerifyTypeCheck { expression, .. } => {
                write!(f, "VerifyTypeCheck({})", expression)
            }
            LeanRequest::VerifyOperationalSemantics { expression, .. } => {
                write!(f, "VerifyOperationalSemantics({})", expression)
            }
            ...
            LeanRequest::VerifyConfigurationValidation { .. } => {
                write!(f, "VerifyConfigurationValidation")
            }
            LeanRequest::ExtractTestCases { specification_file, .. } => {
                write!(f, "ExtractTestCases({})", specification_file)
            }
            LeanRequest::CompareResults { .. } => write!(f, "CompareResults"),
            LeanRequest::RunDifferentialTest { test_case, test_type, .. } => {
                write!(f, "RunDifferentialTest({}, {:?})", test_case, test_type)
            }
            LeanRequest::CheckSpecification { specification_file, .. } => {
                write!(f, "CheckSpecification({})", specification_file)
            }
            LeanRequest::GetVersion => write!(f, "GetVersion"),
            LeanRequest::Ping => write!(f, "Ping"),
            LeanRequest::VerifyTheorem { theorem, .. } => {
                write!(f, "VerifyTheorem({})", theorem)
            }
            LeanRequest::VerifyLemma { lemma, .. } => {
                write!(f, "VerifyLemma({})", lemma)
            }
            LeanRequest::CheckConsistency { .. } => write!(f, "CheckConsistency"),
            LeanRequest::GenerateCounterexample { property, .. } => {
                write!(f, "GenerateCounterexample({})", property)
            }
            LeanRequest::VerifyInvariant { invariant, .. } => {
                write!(f, "VerifyInvariant({})", invariant)
            }
            LeanRequest::VerifyRefinement { .. } => write!(f, "VerifyRefinement"),
        }
    }
}
...
/// Verification context for requests
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Differential test type enumeration
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
/// Specification check type enumeration
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
/// Consistency check type enumeration
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
impl VerificationResponse {
    ...
    pub fn is_success(&self) -> bool {
        match &self.response {
            LeanResponse::VerificationSuccess { .. } => true,
            LeanResponse::TestCasesExtracted { .. } => true,
            LeanResponse::ResultsComparison { match_result, .. } => *match_result,
            LeanResponse::DifferentialTestResult { success, .. } => *success,
            LeanResponse::SpecificationCheckResult { success, .. } => *success,
            LeanResponse::Version { .. } => true,
            LeanResponse::Pong => true,
            LeanResponse::TheoremVerificationResult { success, .. } => *success,
            LeanResponse::ConsistencyCheckResult { consistent, .. } => *consistent,
            LeanResponse::CounterexampleResult { found, .. } => *found,
            LeanResponse::InvariantVerificationResult { holds, .. } => *holds,
            LeanResponse::RefinementVerificationResult { valid, .. } => *valid,
            _ => false,
        }
    }
...
pub mod prelude {
    pub use super::{
        LeanRequest, LeanResponse, VerificationRequest, VerificationResponse,
        VerificationContext, TestType, ComparisonType, DifferentialTestType,
        SpecificationCheckType, ConsistencyCheckType, ErrorType, CoverageInfo,
        DetailedComparison, Difference, DifferenceSeverity, PerformanceComparison,
        MemoryUsage, DependencyInfo, TheoremInfo,
    };
} 