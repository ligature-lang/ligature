...
function findLanguageServer(config) {
...
class LigatureErrorHandler {
    ...
    closed() {
        ...
        if (this.restarts <= this.maxRestarts) {
            vscode.window.showInformationMessage(`Ligature language server connection lost. Attempting to restart (${this.restarts}/${this.maxRestarts})...`);
            return { action: 1 }; // Restart
        }
        vscode.window.showErrorMessage('Ligature language server failed to start after multiple attempts. Please check your configuration.');
...
function createServerOptions(config) {
    ...
    if (!fs.existsSync(serverPath) && !path.isAbsolute(serverPath)) {
        vscode.window.showWarningMessage(`Ligature language server not found at: ${serverPath}. Please build the server or update the path in settings.`);
    }
    return {
        run: {
            command: serverPath,
...
function enhanceHoverWithTypeInfo(hover, document, position) {
...
// Enhanced completion with context awareness
function enhanceCompletionWithContext(completion, document, position, _context) {
...
function addFunctionContextCompletions(completion) {
    if (completion && completion.items) {
        // Filter and enhance function-related completions
        completion.items = completion.items.filter((item) => item.kind === vscode.CompletionItemKind.Function ||
            item.kind === vscode.CompletionItemKind.Keyword);
    }
    return completion;
}
// Add type context completions
function addTypeContextCompletions(completion) {
    if (completion && completion.items) {
        // Filter and enhance type-related completions
        completion.items = completion.items.filter((item) => item.kind === vscode.CompletionItemKind.Class ||
            item.kind === vscode.CompletionItemKind.TypeParameter);
    }
    return completion;
}
// Add import context completions
function addImportContextCompletions(completion) {
    if (completion && completion.items) {
        // Filter and enhance import-related completions
        completion.items = completion.items.filter((item) => item.kind === vscode.CompletionItemKind.Module ||
...
async function startLanguageClient() {
    ...
    client = new node_1.LanguageClient('ligature', 'Ligature Language Server', serverOptions, clientOptions);
...
async function showWorkspaceSymbols() {
    const config = getConfiguration();
...
function provideEnhancedSemanticTokens(document, token) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        for (const word of words) {
            ...
            if (startPos !== -1) {
                ...
                if (tokenType !== null) {
                    tokensBuilder.push(new vscode.Range(lineIndex, startPos, lineIndex, startPos + word.length), tokenType, []);
                }
...
function getSemanticTokenType(word, line) {
    // Keywords
...
function provideEnhancedInlayHints(document, range, token) {
    const hints = [];
    const config = getConfiguration();
        ...
        if (config.inlayHints.enableTypeHints) {
            ...
            if (functionMatches) {
                ...
                for (const param of params) {
                    if (param) {
                        const hint = new vscode.InlayHint(new vscode.Position(lineIndex, paramStart + param.length), `: ${inferParameterType(param)}`, vscode.InlayHintKind.Type);
                        hints.push(hint);
                    }
            ...
            if (functionCallMatches && !line.includes('fun ')) {
                ...
                for (const arg of args) {
                    if (arg) {
                        ...
                        if (paramName) {
                            const hint = new vscode.InlayHint(new vscode.Position(lineIndex, argStart), `${paramName}: `, vscode.InlayHintKind.Parameter);
                            hints.push(hint);
                        }
                    }
...
function inferParameterName(functionName) {
...
async function formatDocumentOnSave(_document) {
    const config = getConfiguration();
    if (!config.formatting.enabled)
        return;
    try {
...
async function formatDocumentOnPaste(_document, _change) {
    const config = getConfiguration();
    if (!config.formatting.enabled)
        return;
    try {
...
async function refactorInlineVariable() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document.languageId !== 'ligature') {
...
async function refactorExtractConstant() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document.languageId !== 'ligature') {
...
function generateTestInputs(functionName) {
...
function generateExpectedOutput(functionName) {
...
async function activate(context) {
    ...
    const config = getConfiguration();
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('ligature.restartLanguageServer', restartLanguageServer), vscode.commands.registerCommand('ligature.showDiagnostics', showDiagnostics), vscode.commands.registerCommand('ligature.formatDocument', formatDocument), vscode.commands.registerCommand('ligature.showHover', showHover), vscode.commands.registerCommand('ligature.goToDefinition', goToDefinition), vscode.commands.registerCommand('ligature.findReferences', findReferences), vscode.commands.registerCommand('ligature.renameSymbol', renameSymbol), vscode.commands.registerCommand('ligature.organizeImports', organizeImports), vscode.commands.registerCommand('ligature.showSymbols', showSymbols), vscode.commands.registerCommand('ligature.showWorkspaceSymbols', showWorkspaceSymbols), vscode.commands.registerCommand('ligature.refactorExtractFunction', refactorExtractFunction), vscode.commands.registerCommand('ligature.refactorInlineVariable', refactorInlineVariable), vscode.commands.registerCommand('ligature.refactorExtractConstant', refactorExtractConstant), vscode.commands.registerCommand('ligature.generateTests', generateTests), vscode.commands.registerCommand('ligature.generateDocumentation', generateDocumentation));
    // Register semantic tokens provider for enhanced syntax highlighting
    if (config.semanticHighlighting.enabled) {
        const semanticTokensLegend = new vscode.SemanticTokensLegend(['function', 'variable', 'type', 'keyword', 'string', 'number', 'comment', 'operator'], ['declaration', 'definition', 'readonly', 'static', 'deprecated', 'abstract', 'async', 'modification', 'documentation', 'defaultLibrary']);
        ...
        context.subscriptions.push(vscode.languages.registerDocumentSemanticTokensProvider({ language: 'ligature' }, semanticTokensProvider, semanticTokensLegend));
    }
    ...
    if (config.inlayHints.enabled) {
        ...
        context.subscriptions.push(vscode.languages.registerInlayHintsProvider({ language: 'ligature' }, inlayHintsProvider));
    }
    ...
    if (config.languageServer.enabled) {
        vscode.window.showInformationMessage('Ligature language support activated! Enhanced features are enabled.', 'Show Commands', 'Open Settings').then(selection => {
